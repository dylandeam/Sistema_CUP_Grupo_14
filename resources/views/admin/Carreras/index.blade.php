            ...
            <div class="card card-outline card-primary">
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        <a href="{{ url('admin/carreras/create') }}" class="btn btn-primary">Nueva Carrera</a>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <tbody>
                            ...
                            @foreach($carreras as $carrera)
                                <tr>
                                    ...
                                    <td style="text-align: center">{{ $carrera->sigla }}</td>
                                    <td style="text-align: center">{{ $carrera->nombre }}</td>
                                    <td style="text-align: center">{{ $carrera->cupos_disponibles }}</td>
                                    <td style="text-align: center">
                                        <div class="btn-group" role="group">

                                             {{-- Botón Show (ver detalles) --}}
                                            <a href="{{ url('/admin/carreras/'.$carrera->id) }}" class="btn btn-info btn-sm"><i class="fas fa-eye"></i></a>

                                            {{-- Botón Editar --}}
                                            <a href="{{ url('/admin/carreras/'.$carrera->id.'/edit') }}" class="btn btn-success btn-sm">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            
                                            {{-- Botón Eliminar --}}
                                            <form action="{{ url('/admin/carreras/'.$carrera->id) }}" method="POST" id="miformulario{{ $carrera->id }}" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" onclick="confirmarEliminacion(event, {{ $carrera->id }})" class="btn btn-danger btn-sm">
